...
export default function ShallowRoutingExample() {
  ...
  async function ShowDetails(barcodeKala: any) {
    ...
    if (token == null) {//zare_nk_041022_commented_movaghat(paqkkardani_chon sms nemiad bekhatere sharayete siasiye keshver login ra bikhial mishim baraye moroor)
      ...
      if (span instanceof HTMLElement) {
        span.innerText = "لطفا ابتدا آنلاین شوید1";
      }
    }
    ...
    if (response.ok) {
      ...
      if (result.status != 0) {
        ...
        console.log('041121-parsedList: '+JSON.stringify(parsedList));
      ...
      if (response.status == 401) {
        ...
        if (span instanceof HTMLElement) {
          span.innerText = "لطفا ابتدا آنلاین شوید2";
        }
      }
    }
  }
  ...
  useEffect(() => {
    ...
    async function tempFuncForAsync() {
      ...
      if (token == null) {//zare_nk_041022_commented_movaghat(paqkkardani_chon sms nemiad bekhatere sharayete siasiye keshver login ra bikhial mishim baraye moroor)
        ...
        if (span instanceof HTMLElement) {
          span.innerText = "لطفا ابتدا آنلاین شوید3";
        }
        return;
      } else {
        type InputDataType = {
        ...
        if (response.ok) {
          ...
          if (data.status != 0) {
            ...
            console.log('041121-tempFuncForAsync-result: ' + JSON.stringify(result));   //zare_nk_041121_eslahi
             console.log('041121-tempFuncForAsync-rrefForParsedList.current: ' + refForParsedList.current);  
          ...
          if (response.status == 401) {
            ...
            if (span instanceof HTMLElement) {
              span.innerText = "لطفا ابتدا آنلاین شوید4";
            }
          }
        }
      }
  ...
  async function addToCartInIndex(
    ...
    console.log('041121-currentRow: ' + JSON.stringify(currentRow));
    if (event != null) {
    ...
    if (token == null) {
      ...
      const data = await response.json();
      console.log('041121-datadata: ' + JSON.stringify(data));
      alert('041121_006');
      if (response.ok) {
        alert('041121_007');
        ...
        if (response.status == 401) {
          ...
          if (span instanceof HTMLElement) {
            span.innerText = "لطفا ابتدا آنلاین شوید5";
          }
        }
      }
    }
    ...
    if (token == null) {
      ...
      if (response.ok) {
        ...
        if (response.status == 401) {
          ...
          if (span instanceof HTMLElement) {
            span.innerText = "لطفا ابتدا آنلاین شوید6";
          }
        }
      }
    }
  ...
  function fillRefsAndfillForCartTagsInDetails(
    ...
    // console.log('041121-fillRefsAndfillForCartTagsInDetails: ' + fillRefsAndfillForCartTagsInDetails);
    refForfather.current = father;
